...
void liangBarsky(int x1, int y1, int x2, int y2, int xmin, int ymin, int xmax, int ymax) {
    ...
    drawColoredLine(x1, y1, clippedX1, clippedY1, BLUE);
    drawColoredLine(clippedX1, clippedY1, clippedX2, clippedY2, WHITE);
    drawColoredLine(clippedX2, clippedY2, x2, y2, BLUE);
}
int main() {
    int gd = DETECT, gm;
    initgraph(&gd, &gm, "");
    ...
    liangBarsky(150,150,175,50,xmin,ymin,xmax,ymax);
    liangBarsky(200,350,100,80,xmin,ymin,xmax,ymax);
    liangBarsky(500,450,75,90,xmin,ymin,xmax,ymax);
    getch();
    delay(50000);
    closegraph();
    return 0;
