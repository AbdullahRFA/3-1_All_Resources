...
void scalePoint(int x, int y, float scaleFactor, int& newX, int& newY) {
    newX = round(x * scaleFactor);
    newY = round(y * scaleFactor);
}

void translatePoint(int x, int y, int translateX, int translateY, int& newX, int& newY) {
    newX = x + translateX;
    newY = y + translateY;
}
...
int main() {
    ...
    scalePoint(x1, y1, scaleFactor, x1, y1);
    scalePoint(x2, y2, scaleFactor, x2, y2);

    int translateX = 75;
    int translateY = 0;
    translatePoint(x1, y1, translateX, translateY, x1, y1);
    translatePoint(x2, y2, translateX, translateY, x2, y2);
